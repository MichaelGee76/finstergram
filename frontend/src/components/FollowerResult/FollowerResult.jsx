...
const FollowerResult = ({ myFollower, handleFollowUpdate }) => {
    ...
    return (
        <div className="post_upper search_res_div">
            <Link
                to={`/profile/${myFollower.userId._id}`}
                className="post_user_infos"
            >
                <img
                    src={myFollower.userId.profilePicture}
                    alt="profile-Picture"
                />

                <div>
                    <h3 className="username_post">
                        {myFollower.userId.userName}
                    </h3>
                    <p className="userdescription_post">
                        {myFollower.userId.profession}
                    </p>
                </div>
            </Link>
            ...
            {followUpdate ? (
                <button
                    onClick={() => deleteFollowing(myFollower.userId._id)}
                ...
                <button
                    onClick={() => postFollowing(myFollower.userId._id)}
