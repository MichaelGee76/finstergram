...
const getOnePostCtrl = asyncHandler(async (req, res) => {
    // const authenticatedUserId = req.authenticatedUserId; brauchen wir hier eigentlich nicht
    const postId = req.params.postId;
    const result = await PostServices.getOnePost(postId);
    if (!result) {
        res.status(500).json("Could not get post");
    }
    sendResponse(res, result);
});

const getDiscoverFeedCtrl = asyncHandler(async (req, res) => {
    const authenticatedUserId = req.authenticatedUserId;

    const result = await PostServices.getDiscoverFeed(authenticatedUserId);
    if (!result) {
        res.status(500).json("Could not load DiscoverFeed ");
    }
    sendResponse(res, result);
});
